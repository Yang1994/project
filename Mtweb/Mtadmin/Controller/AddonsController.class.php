...
class AddonsController extends AdminController {
    ...
    public function _initialize(){
        $this->assign('_extra_menu',array(
            '已装插件后台'=> D('PhpAddons')->getAdminList(),
        ));
        parent::_initialize();
    }
    ...
    public function preview($output = true){
        ...
        if($data['has_adminlist'] && $custom_adminlist){
            $custom_adminlist = '{$custom_adminlist}';
...
str;
    ...
    public function build(){
        ...
        if($data['has_outurl']){
            $addonController = file_put_contents("{$addon_dir}Controller/{$data['info']['name']}Controller.class.php", $addonController);
            $addonModel = file_put_contents("{$addon_dir}Model/{$data['info']['name']}Model.class.php", $addonModel);
        }
    ...
    public function adminList($name){
        ...
        if(!isset($map))
            $map = array();
        if(isset($model))
            $list = $this->lists(D("PhpAddons://{$model}/{$model}")->field($fields),$map);
    ...
    public function saveConfig(){
        $id     =   (int)I('id');
        $config =   I('config');
        $flag = M('PhpAddons')->where("id={$id}")->setField('config',json_encode($config));
        if($flag !== false){
    ...
    public function execute($_addons = null, $_controller = null, $_action = null){
        ...
        if(!empty($_addons) && !empty($_controller) && !empty($_action)){
		   $Addons= A("Mtadmin/File")->ke_upimg();
