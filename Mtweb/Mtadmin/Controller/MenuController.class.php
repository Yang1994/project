...
class MenuController extends AdminController {
    ...
    public function index(){
        $pid  = I('get.pid',0);
        if($pid){
            $data = M('PhpMenu')->where("id={$pid}")->field(true)->find();
            $this->assign('data',$data);
        }
        $title      =   trim(I('get.title'));
        $type       =   C('CONFIG_GROUP_LIST');
        $all_menu   =   M('PhpMenu')->getField('id,title');
        $map['pid'] =   $pid;
        if($title)
            $map['title'] = array('like',"%{$title}%");
        $list       =   M("PhpMenu")->where($map)->field(true)->order('sort asc,id asc')->select();
    ...
    public function add(){
        if(IS_POST){
            ...
            $menus = M('PhpMenu')->field(true)->select();
    ...
    public function edit($id = 0){
        if(IS_POST){
            ...
            $info = array();
            /* 获取数据 */
            $info = M('PhpMenu')->field(true)->find($id);
            $menus = M('PhpMenu')->field(true)->select();
            $menus = D('Common/Tree')->toFormatTree($menus);
    ...
    public function toogleHide($id,$value = 1){
        $this->editRow('PhpMenu', array('hide'=>$value), array('id'=>$id));
    }

    public function toogleDev($id,$value = 1){
        $this->editRow('PhpMenu', array('is_dev'=>$value), array('id'=>$id));
    }

    public function importFile($tree = null, $pid=0){
        if($tree == null){
            $file = APP_PATH."Mtadmin/Conf/Menu.php";
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = M('PhpMenu')->where($map)->field('id,title')->order('sort asc,id asc')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = M('PhpMenu')->where(array('id'=>$value))->setField('sort', $key+1);
            }
            if($res !== false){
                $this->success('排序成功！');
            }else{
